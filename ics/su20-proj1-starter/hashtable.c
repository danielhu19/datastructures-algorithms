...
void insertData(HashTable *table, void *key, void *data) {
  ...
  /* precheck */
  if (table == NULL || key == NULL || data == NULL) return;
  ...
  h = table->hashFunction(key) % table->size;
  newBucket->next = table->data[h];
  table->data[h] = newBucket;
}
...
void *findData(HashTable *table, void *key) {
  ...
  for (bucket = table->data[table->hashFunction(key) % table->size];
